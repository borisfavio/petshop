...
class Reporte extends FPDF{    
   ...
   function drawTable($header, $data){
      ...
      foreach($data as $row){
         $this->Cell($w[0], 8, utf8_decode($row['id_empleado']), 1, 0, 'LR');
         $this->Cell($w[1], 8, utf8_decode($row['nombre_completo']), 1, 0,'LR');
         $this->Cell($w[2], 8, utf8_decode($row['ci_empleado']), 1, 0, 'C');
         $this->Cell($w[3], 8, utf8_decode($row['direccion_empleado']), 1, 0, 'C');
         $this->Cell($w[4], 8, utf8_decode($row['nombre_cargo']), 1, 0, 'C');
         $this->Ln();
      }
   }    
}
   ...
   $reporte->Cell(0, 10, utf8_decode("REPORTE EMPLEADOS"), 0 ,0, "C");
   $header = array("ID", "NOMBRE", "CI","DIRECCION","CARGO");

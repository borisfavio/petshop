...
class Reporte extends FPDF{    
   ...
   function drawTable($header, $data){
      ...
      foreach($data as $row){
         $this->Cell($w[0], 8, utf8_decode($row['id_proveedor']), 1, 0, 'LR');
         $this->Cell($w[1], 8, utf8_decode($row['empresa']), 1, 0,'LR');
         $this->Cell($w[2], 8, utf8_decode($row['contacto']), 1, 0, 'C');
         $this->Cell($w[3], 8, utf8_decode($row['mail']), 1, 0, 'C');
         $this->Ln();
      }
   }    
}
   ...
   $reporte->Cell(0, 10, utf8_decode("REPORTE PROVEEDOR"), 0 ,0, "C");
   $header = array("ID", "EMPRESA", "CONTACTO","CORREO");

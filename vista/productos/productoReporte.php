...
class Reporte extends FPDF{    
   ...
   function drawTable($header, $data){
      ...
      foreach($data as $row){
         $this->Cell($w[0], 20, utf8_decode($row['id']), 1, 0, 'LR');
         $this->Cell($w[1], 20, utf8_decode($row['nombreproducto']), 1, 0,'LR');
         $this->Cell($w[2], 20, utf8_decode($row['nombre_categoria']), 1, 0, 'C');
         $this->Cell($w[3], 20, utf8_decode($row['empresa']), 1, 0, 'C');
         $this->Cell($w[4], 20, utf8_decode($row['costoventa']), 1, 0, 'C');
         $this->Cell($w[5], 20, utf8_decode($row['stock']), 1, 0, 'C');
         // $this->Cell($w[6], 15, $this->Image("imagenesproducto/".$row['imagen_producto'], $this->GetX(), $this->GetY(),10), 1);

         $this->Cell($w[6], 20, $this->Image("../../vista/productos/imagenesproducto/".$row['imagen_producto'], $this->GetX(), $this->GetY(),20), 1);
         //$this->Image("imagenesproducto/".$row['imagen_producto'], 25);
         $this->Ln();
      }
   ...
   $reporte->Cell(0, 10, utf8_decode("REPORTE PRODUCTOS"), 0 ,0, "C");
   $header = array("ID", "NOMBRE", "CATEGORIA","PROVEEDOR","COSTO","STOCK","IMAGEN");
